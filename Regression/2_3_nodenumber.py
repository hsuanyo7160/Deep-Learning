...
def training(n1,n2):
    ...
    class NeuralNetwork:
        def __init__(self):
            self.layer1 = Layer(input_dim=X_train.shape[1], output_dim=n1, activation='relu')  # First hidden layer with 15 neurons
            self.layer2 = Layer(input_dim=n1, output_dim=n2, activation='swish')  # Second hidden layer with 10 neurons
            self.output_layer = Layer(input_dim=n2, output_dim=1, activation='sigmoid')  # Output layer with 1 neuron for binary classification
...
for i in range(1,41,5):
    ...
    for j in range(1,41,5):
        ...
        test_accuracy = 0
        for k in range(3):
            train_accuracy_,test_accuracy_ = training(i,j)
            train_accuracy += train_accuracy_
            test_accuracy += test_accuracy_
        train_accuracy = train_accuracy/3
        test_accuracy = test_accuracy/3
        results[i].append(test_accuracy)
        print(f'layer 1: {i}, layer 2: {j}, Train Accuracy: {train_accuracy:.4f}, Test Accuracy: {test_accuracy:.4f}')

# 繪製圖形
plt.figure(figsize=(10, 6))

# X 軸的 j 值
x_values = list(range(1, 41, 5))

# 繪製每條線
for i in range(1, 41, 5):
    plt.plot(x_values, results[i], label=f'Layer 1 Size: {i}')

# 圖片標題和軸標籤
plt.title('Test Accuracy vs Layer 2 Neurons for Different Layer 1 Sizes')
plt.xlabel('Number of Neurons in Layer 2')
plt.ylabel('Test Accuracy')

# 顯示圖例
plt.legend(title='Layer 1 Size')
